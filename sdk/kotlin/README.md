...
and never run Zig or the NDK. This SDK owns capture ingress, GPU surface handoff,
and platform tracking; the frame graph, lens runtime, and effect pipeline live in
the core.

The [demo](demo/) is a full working reference for the frame loop, and the full
cross-platform capability tour is in the [root README](../../README.md#what-you-get).
...
// build.gradle.kts
dependencies {
    implementation("io.github.avosa:gosslens:0.12.0-alpha.5-alpha.3")
}
```
...
// build.gradle.kts
dependencies {
    implementation("com.github.myzonerocks:gosslens:v0.12.0-alpha.5-alpha.3")
}
```
...
[`MainActivity`](demo/src/main/kotlin/com/gosslens/demo/MainActivity.kt)
...
`submitHands` feeds hands from your own tracker (fill a `GossHandResult` and
`pack()` it), so hand-driven lenses run with no built-in worker.
...
PNG. Full-resolution stills and platform photo formats are iOS-first for now; see
[PARITY.md](../../docs/PARITY.md). `startRecording` opens an MP4 the renderer
appends each rendered frame to, effects baked in, until `stopRecording`; feed the
mic in with `submitAudio` to bake sound into the clip:
