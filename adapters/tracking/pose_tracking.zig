//! Runs the synchronous pose core off the camera thread. Frames arrive NV12 into
//! a latest-wins mailbox; the result leaves through the core's sequence-locked
//! slot. The web tier skips this wrapper and drives pose_core.zig directly, the
//! same split segmentation and the ml rail already take.

const std = @import("std");
const sampler = @import("sampler");
const pose = @import("pose");
const math = @import("math");
const core_mod = @import("pose_core.zig");
...
/// Stands the core up and starts the worker around it.
pub fn create(gpa: std.mem.Allocator, task_bytes: []const u8, threads: i32) CreateError!*PoseTracking {
    ...
    const core = try Core.init(gpa, task_bytes, threads);
...
pub fn readResult(tracking: *PoseTracking, out: *pose.Result) bool {
    return tracking.core.readResult(out);
}
