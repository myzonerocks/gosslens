        ...
        for (0..g.out_channels) |oc| {
            ...
            for (0..g.out_h) |oy| {
                for (0..g.out_w) |ox| {
                    ...
                    for (0..in_per_group) |ic| {
                        ...
                        for (0..g.kernel_h) |ky| {
                            ...
                            for (0..g.kernel_w) |kx| {
                                ...
                                const wv: i64 = @intFromFloat(w.data[((oc * in_per_group + ic) * g.kernel_h + ky) * g.kernel_w + kx]);
                                const inside = iy >= 0 and ix >= 0 and iy < @as(i64, @intCast(g.in_h)) and ix < @as(i64, @intCast(g.in_w));
                                const xv: i64 = if (inside)
                                    @intFromFloat(x.data[((b * g.in_channels + src_c) * g.in_h + @as(usize, @intCast(iy))) * g.in_w + @as(usize, @intCast(ix))])
                                else
                                    x_zero;
                                acc += (xv - x_zero) * (wv - wz);
                            }
                    ...
                    const dst = ((b * g.out_channels + oc) * g.out_h + oy) * g.out_w + ox;
                    if (dequant) |d| {
                        ...
                        const real = (@as(f32, @floatFromInt(acc)) + bias) * d.x_scale * ws;
                        out.data[dst] = out_dtype.clamp(@round(real / d.y_scale + d.y_zero));
                    } else {
                        out.data[dst] = @floatFromInt(acc);
                    }
                }
            }
        }
