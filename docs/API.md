...
| `goss_session_submit_world` | `submitCameraPose(worldFromCamera, projection, timestampUs)`, the bare pose for a host driving a scan with no platform world session behind it: a selfie scan on the front camera, where the depth comes from a lens's own net rather than a sensor | all SDKs |
...
| ABI function | Public operation | Scope |
|---|---|---|
| `goss_session_report_frame` | `reportFrame(frameTimeUs, thermal)`, hands the engine one finished frame's whole-pipeline time and the platform's thermal pressure and returns the rung now in effect. This is the ladder's only clock: deriving the period from camera timestamps was tried and removed, because a 30fps camera sits exactly on a 33.3ms budget and walked every session to passthrough. A pool that turned a frame-path request away counts as pressure here too, whatever the clock said | all SDKs |
| `goss_session_degrade_level` | `degradeLevel()`, the rung in effect. A rung is what the engine is doing, not a label: strides lengthen on face, hand, pose, segmentation and ml.infer, then the segmenter stops, then the beauty chain drops to smoothing and tone, then the lens chain stops and the camera draws straight through. Capture and recording keep their full frame rate on every rung | all SDKs |
...
| `goss_session_define_source` / `goss_session_remove_source` | `defineSource(name)` / `removeSource(name)`, register or drop a named RGBA source for multi-source composition; the camera is the implicit source 0 | all SDKs |
| `goss_session_submit_source_frame_rgba_copy` | `submitSourceFrame(name, rgba, ...)`, uploads one RGBA/BGRA frame into a named source's own texture, from an array or from the buffer a decoder already holds | all SDKs |
| `goss_session_submit_source_frame` | `submitSourceFrame(name, desc, planes)` hands a named source one BGRA or RGBA frame zero-copy, the one plane a platform texture wrapped the way the camera's own frame is, so a second lens composites at no per-frame copy | Swift, Kotlin (native handles); the web SDK copies; on web `submitSourceFrameTexture(name, textureName, width, height)`, the plane a texture the page uploaded and named through `adoptTexture` |
| `goss_session_submit_source_hardware_buffer` | `submitSourceHardwareBuffer(name, desc, buffer)`, a named source's NV12 frame as a platform hardware buffer, the zero-copy ingress the camera already had; without it a second camera or a screen reaches the compositor only through a full RGBA copy, on the target whose buffers are the reason zero-copy exists | Android, where the platform has the buffer |
| `goss_session_set_layout` / `goss_session_clear_layout` | `setLayout(arrangement)` / `clearLayout()`, composites the camera and named sources side-by-side, top-bottom, picture-in-picture, in a grid, or overlaid full-frame and stacked by opacity (Duet, Stitch, live grids, a timeline's transition seam); clear returns to a single camera | all SDKs |
...
| `goss_session_set_geofence` / `goss_session_clear_geofence` | `setGeofence(lat, lon, radius)` / `clearGeofence()`, sets the circle a geofilter lens is active within, derived by the app from the lens's intended place | all SDKs |
| `goss_session_set_geofence_bbox` / `goss_session_set_geofence_polygon` | `setGeofenceBbox(minLat, minLon, maxLat, maxLon)` / `setGeofencePolygon(coords, vertexCount)`, an axis-aligned box or a polygon ring (three to 64 lat/lon vertices) the geofilter lens is active within | all SDKs |
| `goss_session_set_named_geofence` / `goss_session_clear_named_geofences` | `setNamedGeofence(name, lat, lon, radius)` / `clearNamedGeofences()`, named circular regions alongside the default one, so a lens fires `geo.in_region('name')` for its own place among several | all SDKs |
...
| `goss_session_brush_begin` / `goss_session_brush_point` / `goss_session_brush_end` | `beginStroke()` / `addStrokePoint(x, y)` / `endStroke()`, a stroke in normalized screen space | all SDKs |
| `goss_session_brush_undo` / `goss_session_brush_redo` / `goss_session_brush_clear` | `undoStroke()` / `redoStroke()` / `clearStrokes()`, the stroke stacks | all SDKs |
...
| `goss_session_ar_brush_set_style` / `goss_session_ar_brush_set_mode` | `setARBrushStyle(r, g, b, a, width)` / `setARBrushMode(mode)`, the world-anchored brush's style and preset | world-tracking SDKs |
| `goss_session_ar_brush_begin` / `goss_session_ar_brush_point` / `goss_session_ar_brush_end` | `beginARStroke()` / `addARStrokePoint(x, y, z)` / `endARStroke()`, a stroke in the world frame the platform reports poses in; nothing draws without live world tracking | world-tracking SDKs |
| `goss_session_ar_brush_undo` / `goss_session_ar_brush_redo` / `goss_session_ar_brush_clear` | `undoARStroke()` / `redoARStroke()` / `clearARStrokes()`, the world-brush stacks - the same undo/redo pair the screen brush has, so one rail drives both; a fresh stroke drops the redo future | world-tracking SDKs |
...
| `goss_session_flash_risk` | `flashRisk()` reads the photosensitivity risk (0..1) the flash detector last reported for the frames the session was fed, the same value a lens reads as `safety.flash_risk`; the host shows the warning, the engine only measures | all SDKs |
| `goss_session_grab` / `goss_session_release` | `grab(x, y, z)` grabs the nearest dynamic physics body to a world point and drags it there, driving it kinematically so it gathers throw velocity; `release()` lets it fly off dynamic again | all SDKs |
| `goss_session_add_collider` / `goss_session_erase_collider` | `addCollider(x, y, z)` drops a static sphere collider at a world point that content lands on live; `eraseCollider(x, y, z, radius)` removes every live collider within radius - drawing and erasing a 2D collider world | all SDKs |
