...
| `goss_engine_recording_set_realtime` | `startRecording(..., realtime:)`, tells the next recording whether frames arrive in real time; an offline lane passes false so the writer stamps the frames' own timestamps instead of pacing them to the clock | all SDKs |
