...
| `goss_session_ar_brush_undo` / `_redo` / `_clear` | `undoARStroke()` / `redoARStroke()` / `clearARStrokes()`, the world-brush stacks - the same undo/redo pair the screen brush has, so one rail drives both; a fresh stroke drops the redo future | world-tracking SDKs |
