...
| `goss_session_submit_source_frame` | `submitSourceFrame(name, desc, planes)` hands a named source one BGRA or RGBA frame zero-copy, the one plane a platform texture wrapped the way the camera's own frame is, so a second lens composites at no per-frame copy | Swift, Kotlin (native handles); the web SDK copies |
