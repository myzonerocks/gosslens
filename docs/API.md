...
| ABI function | Public operation | Scope |
|---|---|---|
| `goss_session_report_frame` | `reportFrame(frameTimeUs, thermal)`, hands the engine one finished frame's whole-pipeline time and the platform's thermal pressure and returns the rung now in effect. Until a session is reported to, the engine derives the frame period from the camera timestamps instead, so the ladder walks either way; the thermal state has no other source | all SDKs |
