...
| Depth-of-field pass (softens by the host's submitted depth) | proven in the host harness: a depth gradient blurs off the focus plane, with a reference lens and a proof | built, no hardware yet | built |
