...
| Geofilters (location-gated lenses; circle, box, polygon fences, accuracy gate) | proven in the host harness: a geo.in_region trigger fires inside a circle, box or polygon region and not outside, an accuracy gate refuses a vague fix, and only the in-region boolean crosses the trigger rail so the location never leaves the engine | built, no hardware yet | built |
