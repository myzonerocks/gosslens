...
| On-device generative restyle (diffusion img2img and text-to-image, greenscreen and full-face masks, optical-flow temporal coherence, text-to-material, text-to-3D splat cloud) | proven in the host harness: each generative node runs its bundled models off the frame thread and draws through a sprite, mesh, or point cloud, with a proof apiece | built on the inference rail | built on the inference rail |
